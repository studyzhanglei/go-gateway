...
func Grpc(c *gin.Context) {
	...
	client := proto.NewSearchServiceClient(global.GRPC)
	...
	stream.Send(&proto.SearchRequest{
		Request: fmt.Sprintf("你好， 我是你的GRPC回复, %s", G.Username),
	})
	...
	for {
		...
		if err != nil {
			global.RequestLog.Error(err.Error())
		}
	}

	time.Sleep(time.Duration(time.Second * 10))

	//response.FailWithMessage(resp, c)
}
